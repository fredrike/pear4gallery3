...
<? foreach ($children as $i => $child): ?>
<? if(!($child->is_album() || $child->is_movie())): ?>
slideshowImages[<?= $item_no++ ?>] = (['<?= $child->resize_url() ?>', ['<?= url::site("/") ?>', '<?= $child->id ?>'], '<?= $child->width ?>','<?= $child->height ?>', '<?= htmlentities($child->title, ENT_QUOTES) ?>', '<? if (access::can("view_full", $child)) print $child->file_url(); ?>', '<?= $child->url() ?>']);
<? else: ?>
<? $item_no++ ?>
<? endif ?>
<? endforeach ?>
